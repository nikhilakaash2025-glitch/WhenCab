...
export default function ChatWidget({ currentUserId }: { currentUserId: string }) {
  ...
  async function handleBlock() {
    if (!otherUser) return;
    if (!confirm(`Block ${otherUser.name}? You won't see their rides or messages anymore.`)) return;
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      ...
      {isOpen && (
        <div className="bg-white w-80 h-[28rem] rounded-xl shadow-xl border flex flex-col overflow-hidden">
          <div className="bg-blue-600 text-white px-4 py-3 flex items-center justify-between relative">
            ...
            <div className="flex items-center gap-2">
              {activeConversationId && activeConversationId !== "__list__" && (
                <button onClick={() => setShowMenu(!showMenu)} className="text-white/80 hover:text-white text-sm">
                  ⋮
                </button>
              )}
              <button onClick={closeChat} className="text-white/80 hover:text-white text-sm">
                ✕
              </button>
            </div>

            {showMenu && activeConversationId && (
              <div className="absolute top-full right-2 mt-1 bg-white text-gray-800 rounded-lg shadow-lg border text-sm w-40 overflow-hidden z-10">
                <button
                  onClick={() => {
                ...
                <button onClick={handleBlock} className="w-full text-left px-3 py-2 hover:bg-gray-50 text-red-600">
                  Block user
                </button>
              </div>
            )}
          ...
          {showReportForm && (
            <form onSubmit={handleReport} className="border-b p-3 space-y-2 bg-gray-50">
              <select
                value={reportReason}
              ...
              <div className="flex gap-2">
                <button type="submit" className="bg-blue-600 text-white text-xs px-3 py-1.5 rounded">
                  Submit
                </button>
                <button
                  type="button"
          ...
          {(!activeConversationId || activeConversationId === "__list__") && (
            <div className="flex-1 overflow-y-auto">
              {conversations.length === 0 && (
                <p className="text-sm text-gray-500 p-4">No conversations yet. Reach out from a ride post.</p>
              )}
              {conversations.map((c) => {
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-baseline">
                      ...
                      <span className="text-xs text-gray-400">{c.ride.destination}</span>
                    </div>
            ...
            <>
              ...
              <form onSubmit={handleSend} className="border-t p-2 flex gap-2">
                <input
                  ...
                  maxLength={1000}
                  className="flex-1 border rounded-full px-3 py-1.5 text-sm outline-none focus:border-blue-500"
                />
                <button
                  type="submit"
                  disabled={!draft.trim()}
                  className="bg-blue-600 text-white rounded-full px-4 py-1.5 text-sm disabled:opacity-40"
                >
                  Send
                </button>
              </form>
